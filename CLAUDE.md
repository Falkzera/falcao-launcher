...
Roadmap completo (fases 1–5 + empacotamento) na skill. Features atuais: scan + run/stop/logs + detecção de porta + auto-open browser + favicons + busca + atalhos + auto-allocação de portas + override por projeto + esconder/adicionar projetos + abrir VSCode/Ghostty + ícones do tema do sistema + light mode + redesign Falcão System Design + seletor de logo customizada. Próximos prováveis: tray icon, override de script por projeto, suporte a Python/Go (detectar `pyproject.toml`/`go.mod`).
